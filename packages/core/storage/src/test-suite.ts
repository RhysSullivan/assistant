...
export const createStorageContractSuite = (
  name: string,
  config: StorageContractSuiteConfig,
) => {
  describe(`${name} storage contract`, () => {
    ...
    it.effect("filters, sorts, limits, offsets, and projects rows", () =>
      Effect.gen(function* () {
        const storage = yield* config.makeStorage();
        yield* storage.create({ model: "sources", data: { id: "a", scopeId: "s", name: "Alpha", kind: "openapi" } });
        yield* storage.create({ model: "sources", data: { id: "b", scopeId: "s", name: "Beta", kind: "mcp" } });
        yield* storage.create({ model: "sources", data: { id: "c", scopeId: "s", name: "Gamma", kind: "openapi" } });
        ...
        expect(rows).toEqual([{ id: "Beta", name: "Beta" }].map((row) => ({ id: "b", name: row.name })));
      }),
    );
    ...
    it.effect("supports the full where operator set", () =>
      Effect.gen(function* () {
        const storage = yield* config.makeStorage();
        yield* storage.create({ model: "contractValues", data: makeContractValue({ id: "a", label: "Alpha", rank: 1 }) });
        yield* storage.create({ model: "contractValues", data: makeContractValue({ id: "b", label: "Beta", rank: 2 }) });
        yield* storage.create({ model: "contractValues", data: makeContractValue({ id: "c", label: "Gamma", rank: 3 }) });

        expect(yield* ids(storage, [{ field: "rank", operator: "eq", value: 2 }])).toEqual(["b"]);
        expect(yield* ids(storage, [{ field: "rank", operator: "ne", value: 2 }])).toEqual(["a", "c"]);
        expect(yield* ids(storage, [{ field: "rank", operator: "lt", value: 2 }])).toEqual(["a"]);
        expect(yield* ids(storage, [{ field: "rank", operator: "lte", value: 2 }])).toEqual(["a", "b"]);
        expect(yield* ids(storage, [{ field: "rank", operator: "gt", value: 2 }])).toEqual(["c"]);
        expect(yield* ids(storage, [{ field: "rank", operator: "gte", value: 2 }])).toEqual(["b", "c"]);
        expect(yield* ids(storage, [{ field: "rank", operator: "in", value: [1, 3] }])).toEqual(["a", "c"]);
        expect(yield* ids(storage, [{ field: "rank", operator: "not_in", value: [1, 3] }])).toEqual(["b"]);
        expect(yield* ids(storage, [{ field: "label", operator: "contains", value: "mm" }])).toEqual(["c"]);
        expect(yield* ids(storage, [{ field: "label", operator: "starts_with", value: "Al" }])).toEqual(["a"]);
        expect(yield* ids(storage, [{ field: "label", operator: "ends_with", value: "ta" }])).toEqual(["b"]);
      }),
    );
    ...
    it.effect("supports OR where connectors and count", () =>
      Effect.gen(function* () {
        ...
        yield* storage.create({ model: "sources", data: { id: "a", scopeId: "s", name: "Alpha", kind: "openapi" } });
        yield* storage.create({ model: "sources", data: { id: "b", scopeId: "s", name: "Beta", kind: "mcp" } });
        yield* storage.create({ model: "sources", data: { id: "c", scopeId: "s", name: "Gamma", kind: "graphql" } });
    ...
    it.effect("updates and deletes many rows", () =>
      Effect.gen(function* () {
        ...
        expect(yield* storage.count({ model: "contractValues", where: [{ field: "enabled", value: false }] })).toBe(2);
    ...
    it.effect("commits successful transactions", () =>
      Effect.gen(function* () {
        ...
        expect(yield* storage.count({ model: "sources", where: [{ field: "id", value: "committed" }] })).toBe(1);
      }),
    );
    ...
    it.effect("rolls back failed transactions", () =>
      Effect.gen(function* () {
        ...
        const failed = yield* storage
          .transaction((tx) =>
            Effect.gen(function* () {
              yield* tx.create({ model: "sources", data: { id: "a", scopeId: "s", name: "Alpha", kind: "openapi" } });
    ...
    it.effect("fails loudly for missing required fields and duplicate primary keys", () =>
      Effect.gen(function* () {
        ...
        yield* storage.create({ model: "sources", data: { id: "dup", scopeId: "s", name: "One", kind: "openapi" } });
        const duplicate = yield* storage
          .create({ model: "sources", data: { id: "dup", scopeId: "s", name: "Two", kind: "openapi" } })
          .pipe(Effect.either);
...
const ids = (storage: ExecutorStorage, where: Parameters<ExecutorStorage["findMany"]>[0]["where"]) =>
  storage
